...
def multiple_order(request):
    if request.method == 'GET':
        ...
        for c in c:
            i=c.dish_id
            #print(i)
            cart=Cart.objects.filter(hotel_id=hotel_id,cook_status=0)
            n =cart.aggregate(sum('dish_id'))
            print(n)    
...
def test(request):
    if request.session.has_key('chef_mobile'):
        ...
        pending_dish=Cart.objects.filter(hotel_id=e.hotel_id,cook_status=0)
        if pending_dish:
            for pending_dish in pending_dish:
                id=pending_dish.dish_id
                pd=Cart.objects.filter(hotel_id=e.hotel_id,cook_status=0,dish_id=id)
                nqty=0
                if pd:
                    for pd in pd:
                        q=pd.qty
